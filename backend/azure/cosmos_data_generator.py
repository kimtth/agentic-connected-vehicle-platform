...
class CosmosDataGenerator:
    ...
    async def connect(self):
        ...
        try:
            ...
            # Try to get or create POIs and Charging stations containers
            try:
                self.pois_container = self.database.get_container_client(self.pois_container_name)
            except:
                logger.info(f"Creating new container: {self.pois_container_name}")
                self.pois_container = await self.database.create_container(
                    id=self.pois_container_name,
            ...
            try:
                ...
                logger.info(f"Creating new container: {self.charging_stations_container_name}")
                self.charging_stations_container = await self.database.create_container(
                    id=self.charging_stations_container_name,
    ...
    async def generate_status_updates(self, vehicle_id, count, is_electric=False):
        ...
        for i in range(count - 1):
            ...
            status["timestamp"] = (datetime.utcnow() + timedelta(minutes=i*5)).isoformat()
...
if __name__ == "__main__":
    # Import PartitionKey here to avoid issues with circular imports
