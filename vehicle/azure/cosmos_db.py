...
class CosmosDBClient:
                ...
                if self.use_aad_auth:
                    ...
                    try:
                        ...
                        if any(
                            ...
                            logger.warning(
                                "Master key auth failed, attempting AAD fallback"
                            )
    ...
    async def _connect_with_aad(self):
        ...
        try:
            ...
            if os.getenv("ENV_TYPE") == "development":
                self._credential = AzureDeveloperCliCredential(
    ...
    async def get_vehicle_status(self, vehicleId: str) -> Dict[str, Any]:
        ...
        try:
            ...
            items = self.status_container.query_items(
                query=query, parameters=parameters, partition_key=vehicleId
            )
    ...
    async def list_vehicle_status(
        self, vehicleId: str, limit: int = 10
    ) -> List[Dict[str, Any]]:
    ...
    async def subscribe_to_vehicle_status(
        ...
        if not self.status_container:
            logger.warning(
                "Status container not available. Cannot subscribe to vehicle status."
            )
            return
            ...
            while True:
                try:
                    ...
                    if not self.connected or not self.status_container:
                        logger.warning(
                            "Connection lost during status subscription. Attempting to reconnect..."
                        )
                        if not await self.ensure_connected():
                            logger.error(
                                "Failed to reconnect. Ending status subscription."
                            )
                            break
                        if not self.status_container:
                            logger.error(
                                "Status container not available after reconnection."
                            )
                            break
                    ...
                    if not has_updates:
                        ...
                        await asyncio.sleep(
                            1.0
                        )  # Check more frequently if there are updates
                    ...
                    if any(
                        ...
                        logger.info(
                            "Attempting to reconnect due to connection error..."
                        )
                        try:
                            await self.connect()
    ...
    async def update_vehicle_status(
        self, vehicleId: str, status_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def update_command(
        self, command_id: str, vehicleId: str, updated_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def create_notification(
        ...
        try:
            ...
            result = await self.notifications_container.create_item(
    ...
    async def mark_notification_read(
        ...
        try:
            ...
            async for item in items:
                item["read"] = True
                result = await self.notifications_container.replace_item(
                    item=item["id"], body=item, partition_key=item.get("vehicleId")
                )
                return result
